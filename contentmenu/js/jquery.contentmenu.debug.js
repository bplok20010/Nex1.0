...
;(function($){
	"use strict";
	$.contentmenu = function(options){
		this.init(options);
	};
	$.contentmenu.fn = {};
	$.contentmenu.fn.extend = function(obj){
		$.extend($.contentmenu.prototype,obj);
	};
	$.contentmenu.extend = function(obj){
		$.extend($.contentmenu,obj);	
	};
	$.contentmenu.extend({
		version : '1.0', 
		...
		getDefaults : function(opt){
			var _opt = {
				...
				containerCss : 'contentmenu-container-border',
				border : true,
				leftBorder : 1, //gird边框大小 注意grid不会生成边框，而是根据你自己在css中定义的边框大小， 系统默认的是1px
				rightBorder : 1,
				topBorder : 1,
				bottomBorder : 1,
				padding : $.contentmenu.padding,
				width : 150,
				height : 250,
	...
	$.contentmenu.fn.extend({
		...
		tpl : function(tpl,data){
			...
			if( tpl in opt.tpl && opt.tpl[tpl] ) {
				...
				html = self[tpl](data);
			} else if( tpl in $.contentmenu._Tpl && $.contentmenu._Tpl[tpl] ) {
				if( $.contentmenu._Tpl[tpl] in _tpl_ ) {
					var render = _tpl_[ $.contentmenu._Tpl[tpl] ];
					html = render(data);
				} else {
					var render = opt.template.compile( $.contentmenu._Tpl[tpl] );
					
					_tpl_[ $.contentmenu._Tpl[tpl] ] = render;
		...
		bind : function(eventType,func,scope){
			...
			event[eventType] = $.contentmenu._undef(event[eventType],[]);
		...
		unbind : function(eventType,id){
			var self = this;
			var event = self.configs.events;
			var id = $.contentmenu._undef(id,false);
			if(id === false) {
		...
		fireEvent : function(eventType,data){
			...
			//var scope = $.contentmenu._undef(scope,self);
			var data = $.contentmenu._undef(data,[]);
			var r = true;
			if($.isArray(events) ) {
		...
		loadPuglins : function(){
			var self = this;
			$.each( $.contentmenu.puglins,function(i){
				if( $.isFunction( this ) )
					this.call(self);									
			} );
		},
		...
		textLimit : function(text,width,fontSize) {
			var self = this;
			var opt = self.configs;
			var text = $.contentmenu._undef(text,"");
			if(text == "") return text;
	...
	$.contentmenu.fn.extend({
		init : function(options) {
			...
			self.configs = 	$.extend({},$.contentmenu.getDefaults(),options);
			var opt = self.configs;
			opt.self = self;
