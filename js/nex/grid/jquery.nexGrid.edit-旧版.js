...
;(function($){
	...
	$.nexGrid.fn.extend({
		...
		insertEditor : function(rid,field){//创建一个容器定位到指定单元格处
			...
			editor.addClass('edit-cell edit-cell-'+field);
			editor.css({
		...
		resizeEditor : function(rid,field){
			...
			if( !arguments.length ) {
				for( var i=0;i<list.length;i++ ) {
					...
					var tid = opt.id+"_"+editor.field+"_row_"+editor.rid+"_td";
					var td = $("#"+tid);
					var ctd = $("#"+editor.id);
					...
					var cellId = opt.id+"_"+editor.field+"_row_"+editor.rid+"_cell";
					var width = edit.limitWidth ? edit.limitWidth : $("#"+cellId).width();
					editor.editorProvider.C('width',width);
					editor.editorProvider.fireEvent('onSizeChange',[]);
